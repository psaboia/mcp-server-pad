    ...
    if isinstance(raw_data, list):
        for card in raw_data:
            transformed_list.append({
                ...
                "camera": card.get("camera_type_1"),
                "quantity": card.get("quantity"),
                "sample_id": card.get("sample_id"),
                "project_id": card.get("project_id"),
                "issue_id": card.get("issue_id"),
                "processed_file_location": card.get("processed_file_location")
            })
...
async def get_card_image_by_id(card_id: int) -> dict[str, Any]:
    ...
    Steps:
      ...
      5) Processes and resizes the image if needed
      6) Saves the processed image to disk
    ...
    # Return standardized structure with width and height info
    w, h = img.size
    return {
        "success": True,
        "data": {
        ...
        "error": "",
        "description": "Successfully retrieved and processed PAD card image. Image has been resized if needed while maintaining aspect ratio."
    }
...
def multiply_coordinates(data: Any, factor: float) -> Any:
    ...
    Args:
        data: The input data structure (dict or list)
        factor: The multiplication factor
        
    Returns:
        The modified data structure with scaled coordinates
    """
    if isinstance(data, dict):
        for key, value in data.items():
            if key in ["x", "y"] and isinstance(value, (int, float)):
                ...
                multiply_coordinates(value, factor)
    elif isinstance(data, list):
        for item in data:
            multiply_coordinates(item, factor)
    return data
